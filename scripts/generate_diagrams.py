...
def diagram_fl():
    ...
    for i, (label, x) in enumerate([("board-alpha", 80), ("board-beta", 380), ("board-gamma", 680)]):
        ...
        box(d, x, 520, 200, 60, "FL client.py", fill=TEAL, sub=f"machine_{i+1}.csv")
...
def seq_fl_round():
    ...
    _seq_msg(d, xs, y + 55, 1, 1, "local train machine_1.csv", "2")
    _seq_msg(d, xs, y + 110, 2, 2, "local train machine_2.csv", "2")
    _seq_msg(d, xs, y + 165, 3, 3, "local train machine_3.csv", "2")
