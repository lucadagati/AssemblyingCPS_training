...
if [ -f /etc/apache2/conf-available/wot-ws-proxy.conf ]; then
  # Before openstack-dashboard.conf so ProxyPassMatch beats WSGIScriptAlias /horizon
  ln -sf /etc/apache2/conf-available/wot-ws-proxy.conf /etc/apache2/conf-enabled/00-wot-ws-proxy.conf
fi
