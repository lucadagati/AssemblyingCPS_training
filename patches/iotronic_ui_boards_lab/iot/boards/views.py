...
class CreateLabView(forms.ModalFormView):
    template_name = 'iot/boards/create_lab.html'
    modal_header = _("Create LR Container")
    form_id = "create_lab_board_form"
    form_class = project_forms.CreateLabBoardForm
    submit_label = _("Create LR Container")
    submit_url = reverse_lazy("horizon:iot:boards:create_lab")
    success_url = reverse_lazy('horizon:iot:boards:index')
    page_title = _("Create LR Container")
...
class BoardLogsAjaxView(View):
    """JSON tail of LR docker logs for a single board."""
...
class BoardNetworkAjaxView(View):
    """JSON network interfaces / routes for a single board LR."""
