...
urlpatterns = [
    ...
    url(r'^create_lab/$', views.CreateLabView.as_view(), name='create_lab'),
    ...
    url(r'^(?P<board_id>[^/]+)/detail/$', views.BoardDetailView.as_view(),
        name='detail'),
    url(r'^(?P<board_id>[^/]+)/logs_ajax/$', views.BoardLogsAjaxView.as_view(),
        name='logs_ajax'),
    url(r'^(?P<board_id>[^/]+)/network_ajax/$',
        views.BoardNetworkAjaxView.as_view(), name='network_ajax'),
]
