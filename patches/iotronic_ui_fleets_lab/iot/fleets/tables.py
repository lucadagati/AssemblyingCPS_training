...
class ManageFleetMembersLink(tables.LinkAction):
    name = "members"
    verbose_name = _("Manage members")
    url = "horizon:iot:fleets:members"
    classes = ("ajax-modal",)
    icon = "pencil"


class FleetInjectPluginLink(tables.LinkAction):
    name = "inject"
    verbose_name = _("Inject plugin on fleet")
    url = "horizon:iot:fleets:inject"
    classes = ("ajax-modal",)
    icon = "download"


class FleetStartPluginLink(tables.LinkAction):
    name = "start"
    verbose_name = _("Start plugin on fleet")
    url = "horizon:iot:fleets:start"
    classes = ("ajax-modal",)
    icon = "play"


class FleetStopPluginLink(tables.LinkAction):
    name = "stop"
    verbose_name = _("Stop plugin on fleet")
    url = "horizon:iot:fleets:stop"
    classes = ("ajax-modal",)
    icon = "stop"


class FleetCallPluginLink(tables.LinkAction):
    name = "call"
    verbose_name = _("Call plugin on fleet")
    url = "horizon:iot:fleets:call"
    classes = ("ajax-modal",)
    icon = "phone"


class FleetRemovePluginLink(tables.LinkAction):
    name = "remove_plugin"
    verbose_name = _("Remove plugin from fleet")
    url = "horizon:iot:fleets:remove_plugin"
    classes = ("ajax-modal",)
    icon = "trash"
...
class FleetsTable(tables.DataTable):
    ...
    class Meta(object):
        ...
        row_actions = (
            EditFleetLink,
            ManageFleetMembersLink,
            FleetInjectPluginLink,
            FleetStartPluginLink,
            FleetStopPluginLink,
            FleetCallPluginLink,
            FleetRemovePluginLink,
            DeleteFleetsAction,
        )
