...
class CreateView(forms.ModalFormView):
    ...
    submit_url = "horizon:iot:plugins:create"
    success_url = reverse_lazy('horizon:iot:plugins:index')
    cancel_url = reverse_lazy('horizon:iot:plugins:index')
    page_title = _("Create Plugin")

    def get_context_data(self, **kwargs):
        context = super(CreateView, self).get_context_data(**kwargs)
        context['plugin_ide_mode'] = 'create'
        context['submit_url'] = reverse(self.submit_url)
        context['cancel_url'] = self.cancel_url
        return context

    def get_initial(self):
        # Default = sync HelloName (Callable ON). LR expects Worker(Plugin.Plugin) + run().
        # class Worker(Plugin.Plugin) + run(); PluginCall is a cloud RPC, not a method.
        return {
            'callable': True,
            'code': (
                ...
                '    """Synchronous plugin: Callable ON, then Call (PluginCall RPC)."""\n'
                '\n'
...
class UpdateView(forms.ModalFormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['submit_url'] = reverse(self.submit_url, args=args)
        context['cancel_url'] = self.cancel_url
        context['plugin_ide_mode'] = 'update'
        return context
